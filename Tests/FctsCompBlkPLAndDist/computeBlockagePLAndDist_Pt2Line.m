function [ blockagePL, blockageDistInM ] ...
    = computeBlockagePLAndDist_Pt2Line(txXYAlt, rxXYAlt, lidarProfile, simConfigs)
...
if FLAG_DEBUG
    ...
    boolsClearType3_1 = boolsBlockedType3_1 == false;
    boolsClearType2 = boolsBlockedType2 == false;
    ...
    curMarker = '.m'; hBlocked3_2 = plot(nan, nan, curMarker);
    plot(obsLidarProfDists(boolsBlocked), ...
        obsLidarProfileZs(boolsBlocked), curMarker);
    curMarker = '.y'; hClear3_1 = plot(nan, nan, curMarker);
    plot(obsLidarProfDists( ...
        boolsClearType3_1 & (isnan(boolsBlockedType2))), ...
        obsLidarProfileZs( ...
        boolsClearType3_1 & (isnan(boolsBlockedType2))), curMarker);
    curMarker = '.c'; hClear2 = plot(nan, nan, curMarker);
    plot(obsLidarProfDists( ...
        boolsClearType2 & boolsBelowDirectPath), ...
        obsLidarProfileZs( ...
        boolsClearType2 & boolsBelowDirectPath), curMarker);
    curMarker = '.r'; hBlocked1 = plot(nan, nan, curMarker);
    plot(obsLidarProfDists(~boolsBelowDirectPath), ...
        obsLidarProfileZs(~boolsBelowDirectPath), curMarker);
    % axis equal;
    legend([hTx, hRx, hLoS, hProf, ...
        hBlocked1, hClear2, hClear3_1, hBlocked3_2], ...
        'Tx', 'Rx', 'LoS Path', 'Profile', ...
        'Blockage (i)', 'Clearance (ii)', ...
        'Clearance (iii-1)', 'Blockage (iii-2)', ...
        'Location', 'northwest');
