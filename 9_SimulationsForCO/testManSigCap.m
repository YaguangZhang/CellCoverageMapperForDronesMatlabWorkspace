...
if flagGenKmlOverview
    ...
    for idxPreset = 1:numOfPresets
        ...
        maxAllowedTimeGapInS = ...
            cachedResultsCell{idxPreset}.MAX_ALLOWED_TIME_GAP_IN_S;
        ...
        switch lower(MARKER_STYLE)
            ...
            case lower('Line')
                ...
                for idxT = 1:curNumOfTracks
                    ...
                    curBoolsIsAtt = boolsIsAtt(curPtIndices);
                    curBoolsIsVer = boolsIsVer(curPtIndices);
                    curBoolsIsTM = boolsIsTM(curPtIndices);

                    % Note that we may need to break the track again
                    % because the time gaps change when we filter by
                    % carriers.
                    if any(curBoolsIsAtt)
                        gePlotsAtt = strcat(gePlotsAtt, ...
                            genKmlStrForGpsTrackAs3DLines( ...
                            allLons(curBoolsIsAtt), ...
                            allLats(curBoolsIsAtt), ...
                            ones(size(allLons( ...
                            curBoolsIsAtt))).*lineHInMMinus, ...
                            allTimestampsDatetimeUtc(curBoolsIsAtt), ...
                            maxAllowedTimeGapInS, ...
                            ['Track No. ', num2str(idxT)], ...
                            lineColorAtt, lineWidthPlus, lineExtrude));
                    end
                    if any(curBoolsIsVer)
                        gePlotsVer = strcat(gePlotsVer, ...
                            genKmlStrForGpsTrackAs3DLines( ...
                            allLons(curBoolsIsVer), ...
                            allLats(curBoolsIsVer), ...
                            ones(size(allLons( ...
                            curBoolsIsVer))).*lineHInM, ...
                            allTimestampsDatetimeUtc(curBoolsIsVer), ...
                            maxAllowedTimeGapInS, ...
                            ['Track No. ', num2str(idxT)], ...
                            lineColorVer, lineWidth, lineExtrude));
                    end
                    if any(curBoolsIsTM)
                        gePlotsTM = strcat(gePlotsTM, ...
                            genKmlStrForGpsTrackAs3DLines( ...
                            allLons(curBoolsIsTM), ...
                            allLats(curBoolsIsTM), ...
                            ones(size(allLons( ...
                            curBoolsIsTM))).*lineHInMPlus, ...
                            allTimestampsDatetimeUtc(curBoolsIsTM), ...
                            maxAllowedTimeGapInS, ...
                            ['Track No. ', num2str(idxT)], ...
                            lineColorTM, lineWidthMinus, lineExtrude));
                    end
                end
