...
if ~flagHistResAvailable
    ...
    % Replace LiDAR z values with ground elevation for user-specified area.
    if isfield(simConfigs, 'polyXYsToUseGroundEle')
        boolsToUseGroundEle = inpolygon( ...
            curTerrainProfileSampUtmLocs(:, 1), ...
            curTerrainProfileSampUtmLocs(:, 2), ...
            simConfigs.polyXYsToUseGroundEle(:, 1), ...
            simConfigs.polyXYsToUseGroundEle(:, 2));
        lidarProfile(boolsToUseGroundEle) ...
            = terrainProfile(boolsToUseGroundEle);
    end
