...
% Output:
%   - simState
%       (Note: fields blockageByTerrainDistMaps, blockageByVegDistMaps,
%       pathLossWithVegMaps, blockageByTerrainDistMapsForEachCell,
%       pathLossWithVegMapsForEachCell will be generated only if
%       simConfigs.FLAG_EVAL_LOSS_THROUGH_VEG is set to be true.)
...
%       - mapGridResolutionInM
%         The map grid spatical resolution in meters.
...
%         be inspected.
%       - coverageItmMaps, coverageMapsForEachCellItm
%         Similar to coverageItmMaps and coverageMapsForEachCell, but with
%         path loss values evaluated using the NTIA ITM model (instead of
%         eHata).
...
switch simConfigs.CURRENT_SIMULATION_TAG
    case {'CustomSim', 'WHIN_WEATHER_STATIONS', ...
        ...
        elseif strcmp('UniOfCoBoulderCampus_NorlinQuad_', ...
                simConfigs.CURRENT_SIMULATION_TAG(1:min(32, ...
                length(simConfigs.CURRENT_SIMULATION_TAG))))
            % Load the location information.
            ABS_PATH_TO_SHARED_FOLDER ...
                = evalin('base', 'ABS_PATH_TO_SHARED_FOLDER');
            norlinQuadRxLocTb = readtable( ...
                fullfile(ABS_PATH_TO_SHARED_FOLDER, ...
                'ColoradoSims', '20221202_NorlinQuad', ...
                'RX_locations.csv'));

            simState.mapGridLatLonPts = ...
                [norlinQuadRxLocTb.RXLatitudes, ...
                norlinQuadRxLocTb.RXLongitudes];
        end
        ...
        simState.mapGridResolutionInM ...
            = min(abs([diff(unique(mapGridXs)); diff(unique(mapGridYs))]));

    otherwise
        [simState.mapGridXYPts, simState.mapGridResolutionInM] ...
...
boolsCellAntsToKeep ...
    = inpoly2(cellAntsXYH(:,1:2), utmXYBoundaryToKeepCellTowers);
...
if ~exist([curDirToSave, '.eps'], 'file')
    ...
    [axisLonLatToSet, weightForWidth] ...
        ...
        max(gpsLatsBoundaryToKeepCellTowers)], ...
...
if ~isfield(simState, 'blockageMapsForEachCell')
    ...
    if isfield(simConfigs, 'itmParameters')
        simState.coverageItmMapsForEachCell = cell(numOfEffeCellAnts, 1);
    end
    simState.blockageDistMapsForEachCell = cell(numOfEffeCellAnts, 1);
    ...
    simState.TimeUsedInSForEachPixel = cell(numOfEffeCellAnts, 1);
    ...
    for idxEffeCellAnt = 1:numOfEffeCellAnts
        ...
        assert(length(curAllLocIndices)==length(curUniqueAllLocIndices) ...
            & all(curUniqueAllLocIndices == (1:curNumOfTasks)), ...
            'Invalid locIndicesForAllWorkers!');
        ...
        for idxDroneHeightInM = 1:numOfRxHeightToInspect
            ...
            simState.coverageMapsForEachCell{idxEffeCellAnt} ...
                {idxDroneHeightInM} = nan(1, numPixelsPerMap);
            if isfield(simConfigs, 'itmParameters')
                simState.coverageItmMapsForEachCell{idxEffeCellAnt} ...
                    {idxDroneHeightInM} = nan(1, numPixelsPerMap);
            end
...
for idxEffeCellAnt = nextIdxEffeCellAnt:numOfEffeCellAnts
    ...
    parfor (idxWorker = 1:numOfWorkers, parforArg)
        ...
        for idxDroneLoc = curDroneLocIndices
            ...
            for idxDroneHeightInM = 1:numOfRxHeightToInspect
                ...
                if simConfigs.FLAG_EVAL_LOSS_THROUGH_VEG ...
                        || isfield(simConfigs, 'itmParameters')
                    ...
                    if isfield(simConfigs, 'itmParameters')
                        curCoveragePLItm = extraResults.coveragePLItm;
                    end
                else
                    [curBlockagePL, curCoveragePL, curBlockageDistInM] ...
                        = computeBlockageAndCoveragePLs( ...
                ...
                % First 6 elements: [blockagePL, coveragePL, pixelExecTime,
                ...
                if simConfigs.FLAG_EVAL_LOSS_THROUGH_VEG
                    % The 7th element: curBlockageByTerrainDistInM.
                    resultsFromWorkersCell{idxWorker} ...
                        (curWorkerPixCnt, 7) ...
                        = curBlockageByTerrainDistInM;
                end
                ...
                if isfield(simConfigs, 'itmParameters')
                    % The 8th element: curCoveragePLItm.
                    resultsFromWorkersCell{idxWorker} ...
                        (curWorkerPixCnt, 8) = curCoveragePLItm;
                end
    ...
    for idxResult = 1:numOfResultsFromWorkers
        ...
        if simConfigs.FLAG_EVAL_LOSS_THROUGH_VEG
            curBlockageByTerrainDistInMCache = curResult(7);

            % Estimate the vegetable depth.
            curBlockageByVegDistInM = ...
                curBlockageDistInM - curBlockageByTerrainDistInMCache;
            % Estimate the overall path loss considering the vegetation.
            % Note that estimateExcessPL will set negative inputs to zero.
            curPathLossWithVeg = ...
                curCoveragePL + estimateExcessPL(curBlockageByVegDistInM);

            simState.blockageByTerrainDistMapsForEachCell ...
                {idxEffeCellAnt}{curIdxDroneHeightInM} ...
                (curIdxDroneLoc) = ...
                curBlockageByTerrainDistInMCache;
            simState.pathLossWithVegMapsForEachCell{idxEffeCellAnt} ...
        ...
        if isfield(simConfigs, 'itmParameters')
            curCoveragePLItm = curResult(8);

            simState.coverageItmMapsForEachCell{idxEffeCellAnt} ...
                {curIdxDroneHeightInM}(curIdxDroneLoc) = curCoveragePLItm;
        end
    end
...
if isfield(simConfigs, 'itmParameters')
    simState.coverageItmMaps = cell(1, numOfRxHeightToInspect);
end

simState.blockageDistMaps = cell(1, numOfRxHeightToInspect);
if simConfigs.FLAG_EVAL_LOSS_THROUGH_VEG
    [simState.blockageByTerrainDistMaps, ...
...
for idxH = 1:numOfRxHeightToInspect
    ...
    if isfield(simConfigs, 'itmParameters')
        simState.coverageItmMaps{idxH} ...
            = fetchPathlossResultsFromSimState(simState, ...
            1, idxH, 'CoverageItm', simConfigs, 'utm');
    end
    ...
    for idxEffeCell = 2:numOfEffeCellAnts
        ...
        if isfield(simConfigs, 'itmParameters')
            simState.coverageItmMaps{idxH} = min( ...
                simState.coverageItmMaps{idxH}, ...
                fetchPathlossResultsFromSimState(simState, ...
                idxEffeCell, idxH, 'CoverageItm', simConfigs, 'utm'));
        end
        ...
        if simConfigs.FLAG_EVAL_LOSS_THROUGH_VEG
            simState.blockageByTerrainDistMaps{idxH} = min( ...
                simState.blockageByTerrainDistMaps{idxH}, ...
                fetchPathlossResultsFromSimState(simState, ...
                idxEffeCell, idxH, 'BlockageByTerrainDist', ...
                simConfigs, 'utm'));
            simState.blockageByVegDistMaps{idxH} = min( ...
                simState.blockageByVegDistMaps{idxH}, ...
                fetchPathlossResultsFromSimState(simState, ...
                idxEffeCell, idxH, 'BlockageByVegDist', ...
                simConfigs, 'utm'));
            simState.pathLossWithVegMaps{idxH} = min( ...
                simState.pathLossWithVegMaps{idxH}, ...
                fetchPathlossResultsFromSimState(simState, ...
                idxEffeCell, idxH, 'PathLossWithVeg', ...
                simConfigs, 'utm'));
        end
    end
    ...
    if isfield(simConfigs, 'itmParameters')
        simState.coverageItmMaps{idxH} ...
            = simState.coverageItmMaps{idxH}(:,3);
    end
