...
utmXYBoundaryPolyToKeepCellTowers ...
    = polybuffer(polyshape(simConfigs.UTM_X_Y_BOUNDARY_OF_INTEREST), ...
...
utmXYBoundaryToKeepCellTowers = utmXYBoundaryPolyToKeepCellTowers.Vertices;
...
for idxH = 1:numOfRxHeightToInspect
    ...
    simState.blockageByVegDistMaps{idxH} = ...
        simState.blockageDistMaps{idxH}(:,3) ...
        - simState.blockageByTerrainDistMaps{idxH}(:,3);
    % Estimate the overall path loss considering the vegetation. Note that
    % estimateExcessPL will set negative inputs to zero.
    simState.pathLossWithVegMaps{idxH} ...
        = estimateExcessPL(simState.blockageByVegDistMaps{idxH});
    ...
    for idxEffeCell = 2:numOfEffeCellAnts
        ...
        curBlockageDistMap = fetchPathlossResultsFromSimState(simState, ...
            idxEffeCell, idxH, 'BlockageDist', simConfigs, 'utm');
        curBlockageByTerrainDistMap ...
            = fetchPathlossResultsFromSimState(simState, ...
        ...
        simState.blockageByVegDistMaps{idxH} = min( ...
            simState.blockageByVegDistMaps{idxH}, ...
            curBlockageDistMap(:,3) - curBlockageByTerrainDistMap(:,3));
        simState.pathLossWithVegMaps{idxH} = min( ...
            simState.pathLossWithVegMaps{idxH}, ...
            estimateExcessPL(curBlockageDistMap(:,3) ...
            - curBlockageByTerrainDistMap(:,3)));
    end
