            ...
            if ~flagReuseCacheSuccess
                try
                    ...
                    try
                        ...
                        % Convert the coordinates with projection
                        % information from other tiles.
                    ...
                    % Note: the data type is single for elements in
                    % lidarZs.
                    lidarZs = lidarDataImg(:);
                    % Note: the final matrix lidarXYZ has "single"
                    % elements, even though both lidarXs and lidarYs are of
                    % the type "double".
                    ...
                    % Note: the (lon, lat) boundary is based on the UTM (x,
                    % y) result to avoid running the slow command boundary.
                    ...
                    % Create a function to get LiDAR z from UTM
                    % coordinates.
                    ...
                    % For avoid reading in incomplete raw terrain files
                    % that are being downloaded by other worker, we will
                    % try loading the data a few times.
                    numTrials = 0;
                    maxNumTrialsAllowed = 3; % 30;
                    timeToWaitBeforeTryAgainInS = 0; % 30;
                    % Default directory to save USDA data.
                    usdaDataDir = 'usgsdata';
                    % Backup directory to store USDA data when the data in
                    % the default directory do not work (possibly because
                    % of the collision of workers trying to store the same
                    % data file at the same dir). We will assign one folder
                    % for each worker to avoid any future collisions.
                    ...
                    while ~isinf(numTrials)
                        try
                            % Use USGS 1/3 arc-second (~10m) resolution
                            % data for US terrain elevation.
                            ...
                            if strcmpi(err.identifier, ...
                                ...
                                % elevation libary. Some downloaded tiles
                                % have more data than what the raster
                                % needs. Typically, if the size of the
                                % raster is m x n, the data fetched could
                                % be (m+1) x (n+1). We will only use m x n
                                % of the data fetched, then.
                            ...
                            warning(['Error fetching elevation info ', ...
                    ...
                    % Order the raw elevation data so that both lat and lon
                    ...
                    % For very tiny LiDAR data tiles, we may not have
                    % enough elevation data to carry out interp2. If that
                    % happens, we will use the LiDAR data grid for the
                    % elevation, too.
                    try
                        ...
                        % For nan results, fetch the elevation data from
                        % USGS.
                        ...
                        % does not work, please run plot_google_maps with
                        % your Google Maps API once and try again.
