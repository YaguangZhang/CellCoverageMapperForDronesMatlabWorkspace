...
class DDownloader:
    """ A CLASS to download http files recursively
    ...
    async def _start_downloads(self, session: aiohttp.ClientSession, filter: str,
                               extensions: List[str], flagNoDir: bool):
        tasks = []
        for url in self.files_urls:
            if flagNoDir:
                download_directory = self.directory
            else:
                download_directory_parts = [self.directory] + url.split("/")[2:-1]
                download_directory = os.path.join(*download_directory_parts)
    ...
    async def download_files(self, urls: Set[str] = None, filter: Union[str, callable] = None,
                             extensions: List[str] = None, flagNoDir: bool = False):
        """ Download multiple files
            urls:set[str]-> a set of urls files to download
            filter:str -> regex to filter the files that matches it
            extensions:list[str] ->  specify the extensions of the files you want to fetch
            flagNoDir:bool -> whether to flatten the original online directory
        """
        if urls:
            self.files_urls = urls
        connector = aiohttp.TCPConnector(limit=self.workers)
        async with aiohttp.ClientSession(connector=connector) as session:
            await self._start_downloads(session, filter, extensions, flagNoDir)
