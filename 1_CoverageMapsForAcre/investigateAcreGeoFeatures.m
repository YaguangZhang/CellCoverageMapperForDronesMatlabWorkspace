...
if exist(dirToCachedResults, 'file')
    ...
    % Get the ground elevation again for the low-resolution grid via a
    ...
    save(dirToCachedResults, ...
        ...
        'terrainEles', 'lidarZs', ...
        ... Reference ground elevation data.
        'rawRefElevData', ...
...
[lowResGridLatMat, lowResGridLonMat] ...
...
% Interpolate reference elevation data (from 3rd-party library).
[rawRefElevDataLonsSorted, rawRefElevDataLatsSorted, ...
    rawElevDataElesSorted] ...
    = sortGridMatrixByXY(...
    rawRefElevData.longs, rawRefElevData.lats, ...
    rawRefElevData.elev);
% Create a grid for the elevation data.
[usgsRefElevDataLons, usgsRefElevDataLats] = meshgrid( ...
    rawRefElevDataLonsSorted, rawRefElevDataLatsSorted);
% Interperlate the data with lat and lon.
lowResGridTerrainElesRef = interp2( ...
    usgsRefElevDataLons, usgsRefElevDataLats, ...
    rawElevDataElesSorted, lowResGridLonMat, lowResGridLatMat);
highResGridTerrainElesRef = interp2( ...
    usgsRefElevDataLons, usgsRefElevDataLats, ...
    rawElevDataElesSorted, highResGridLonMat, highResGridLatMat);

% There is a bug interpolating USGS elevation data on Linux. Results there
% seem to shift a little when inspected on a map. As a workaround, we will
% run this script on Windows and replaced the elevation results with the
% correct version.
terrainEles = interp2( ...
    usgsRefElevDataLons, usgsRefElevDataLats, ...
    rawElevDataElesSorted, gridLatLonPts(:,2), gridLatLonPts(:,1));
lowResGridTerrainEleMat = lowResGridTerrainElesRef;
[lowResGridDeltaEleXs, lowResGridDeltaEleYs] ...
    = gradient(lowResGridTerrainEleMat);
highResGridTerrainEleMat = highResGridTerrainElesRef;
[highResGridDeltaEleXs, highResGridDeltaEleYs] ...
    = gradient(highResGridTerrainEleMat);

% For easier gradient visualization (i) in ACRE and (ii) in 3D.
boolsLowResGridPtsInAcre = inpoly2( ...
    [lowResGridUtmXMat(:), lowResGridUtmYMat(:)], ...
    UTM_X_Y_BOUNDARY_ACRE);

boolsHighResGridPtsInAcre = inpoly2( ...
...
dispelev(rawRefElevData, 'mode', 'latlong');
set(gcf, 'Position', [0, 0, figureSize], ...
    'PaperPositionMode', 'auto');
colormap turbo; colorbar;
xticks([]); yticks([]); view(2); zlim([0, max(rawRefElevData.elev(:))]);
adjustFigSizeByContent(gcf, axisToSet, ...
    'height', weightForWidth*1.05);
plot_google_map('MapType', 'satellite');
curPathToSaveFig = fullfile(pathToSaveResults, '1_Ele_LowRes_RawRefEle');
...
figure('Position', [0, 0, figureSize], ...
    'PaperPositionMode', 'auto'); colormap turbo;
plot3k([lowResGridLonMat(boolsLowResGridPtsInAcre), ...
    lowResGridLatMat(boolsLowResGridPtsInAcre), ...
    lowResGridTerrainElesRef(boolsLowResGridPtsInAcre)], ...
    'Labels', {'', '', '', '', ...
    'Ground Elevation (m)            '});
xticks([]); yticks([]); view(2); zlim([0, max(rawRefElevData.elev(:))]);
adjustFigSizeByContent(gcf, axisToSet, ...
...
curPathToSaveFig = fullfile(pathToSaveResults, ...
    '1_Ele_LowRes_RefEle_GridPt');
export_fig([curPathToSaveFig, '.jpg'], '-m3');
saveas(gcf, [curPathToSaveFig, '.fig']);

% Plot3k version for high resolution.
figure('Position', [0, 0, figureSize], ...
    'PaperPositionMode', 'auto'); colormap turbo;
plot3k([highResGridLonMat(boolsHighResGridPtsInAcre), ...
    highResGridLatMat(boolsHighResGridPtsInAcre), ...
    highResGridTerrainElesRef(boolsHighResGridPtsInAcre)], ...
    'Labels', {'', '', '', '', ...
    'Ground Elevation (m)            '});
xticks([]); yticks([]); view(2); zlim([0, max(rawRefElevData.elev(:))]);
adjustFigSizeByContent(gcf, axisToSet, ...
...
for idxNoB = 1:length(numsOfBins)
    numOfBins = numsOfBins(idxNoB); figure;
    histogram(highResGridDeltaEleAbsVs(boolsHighResGridPtsInAcre), ...
        numOfBins, 'Normalization', 'probability');
    xlabel('Slope (Ground Elevation Gradient Magnitude)');
    ylabel(['Relative Frequency (', num2str(numOfBins), ' Bins)']);
    grid on; grid minor;
    export_fig(fullfile(pathToSaveResults, ...
...
for idxNoG = 1:length(numsOfGroups)
    ...
    hQuiver = quiver3(lowResGridLonMat(boolsLowResGridPtsInAcre), ...
        lowResGridLatMat(boolsLowResGridPtsInAcre), ...
        ones(size(lowResGridLonMat(boolsLowResGridPtsInAcre))) ...
        .*max(terrainElesToShow)+2,...
        -lowResGridDeltaEleXs(boolsLowResGridPtsInAcre), ...
        -lowResGridDeltaEleYs(boolsLowResGridPtsInAcre), ...
        zeros(size(lowResGridLonMat(boolsLowResGridPtsInAcre))), 'b');
...
hQuiver = quiver(lowResGridLonMat(boolsLowResGridPtsInAcre), ...
    lowResGridLatMat(boolsLowResGridPtsInAcre), ...
    -lowResGridDeltaEleXs(boolsLowResGridPtsInAcre), ...
    -lowResGridDeltaEleYs(boolsLowResGridPtsInAcre), 'w');
title('Ground Elevation: Slope (Negative Gradient)');
set(gcf, 'Position', [0, 0, figureSize]);
xlabel(''); ylabel(''); xticklabels(''); yticklabels('');
adjustFigSizeByContent(gcf, axisToSet, ...
    'height', weightForWidth*1.05);
...
disp(' ')
disp(['    [', datestr(now, datetimeFormat), ...
    '] Ground elevation gradient magnitude: raw elevation overview ...'])
elesToPlot = highResGridTerrainEleMat(boolsHighResGridPtsInAcre);
figure('Position', [0, 0, figureSize], ...
    'PaperPositionMode', 'auto'); colormap turbo;
plot3k([highResGridLonMat(boolsHighResGridPtsInAcre), ...
    highResGridLatMat(boolsHighResGridPtsInAcre), elesToPlot]);
title('Ground Elevation for Gradient Magnitude (m)');
xticks([]); yticks([]); view(2);
adjustFigSizeByContent(gcf, axisToSet, ...
    'height', weightForWidth*1.05);
plot_google_map('MapType', 'satellite');
export_fig(fullfile(pathToSaveResults, ...
    '1_Ele_HighRes_GridPts.jpg'), '-m3');
